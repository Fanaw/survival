...
public class Creature extends Entite{
	...
	public void move(){ // CHECK LA COLLISION AVEC UNE ENTITE AU TICK D'APRES, SI IL N'Y A PAS COLLISION ON PEUT BOUGER
	...
	public void moveX(){ // CHECK LA COLLISION AVEC LES TILES AU TICK D'APRES EN FONCTION DU DEPLACEMENT SOUHAITE
		
		if(xMove > 0){//Moving right
			int tx = (int) (x + xMove + bords.x + bords.width) / Tile.largeur; // VARIABLE TEMPORAIRE CORRESPONDANT AU DEPLACEMENT SOUHAITE
			
			if(!collisionTile(tx, (int) (y + bords.y) / Tile.hauteur) && !collisionTile(tx, (int) (y + bords.y + bords.height) / Tile.hauteur)){
				x += xMove;
			}else{
	...
	protected boolean collisionTile(int x, int y){ // BOOLEAN EN FONCTION DE LA SOLIDITE DES LA TILE
